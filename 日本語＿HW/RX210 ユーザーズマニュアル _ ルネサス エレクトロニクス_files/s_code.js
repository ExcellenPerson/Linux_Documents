...
var s_account = "renesasresource"
//var s_account = "renesasrenesas2012test"
...
s.linkDownloadFileTypes = "ads,apk,avi,binary,bsd,bsdl,cir,db,doc,docx,dxf,eps,exe,ibs,lzh,mov,mp3,mpg,msi,pdf,pps,ppt,pptx,step,stp,swf,swz,wav,wmv,xls,xlsx,zip"
s.linkInternalFilters = "javascript:,resource.renesas.com"
...
function s_doPlugins(s) {
    ...
    s.campaign = s.getQueryParam("campaign,cid", ":");
    ...
    s._ttlidx = document.title.indexOf("|");
    if (!s._ttlidx) {
        s._ttlidx = document.title.indexOf("\uff5c");
    }
    s.prop32 = document.title.substring(0, s._ttlidx);
    s.prop32 = s.prop32.replace(/(^\s*)|(\s*$)/g, "");
    if (!s.prop32) {
        s.prop32 = document.title;
    }


    // Current URL
    // prop33 , prop65(Survey FAQ useful)
    ...
    // old impl
    s.arrPath = location.pathname.replace(/;jsessionid=[^\?]+/i, "").split("/");
    s.intPathLen = s.arrPath.length;
    s.arrPathNew = new Array();
    s.strPathNew = "";
    s.strFile = "";
    s.strFileNew = "";
    if (s.intPathLen > 2) {
        for (s.intIdx = 1; s.intIdx < (s.intPathLen - 1); s.intIdx++) {
            s.arrPathNew.push(s.arrPath[s.intIdx]);
        }
        s.strPathNew = s.arrPathNew.join("/");
    }
    if (s.strPathNew.length > 66) {
        s.strPathNew = s.strPathNew.substring(0, 66);
    }
    s.strFile = s.arrPath[s.intPathLen - 1];
    s.strFileNew = s.strFile.replace(/\.\w+$/, "");
    if (s.strFileNew.length > 26) {
        s.strFileNew = s.strFileNew.substring(0, 26);
    }
    ...
    s.prop54 = location.pathname.replace(/;jsessionid=[^\?]+/i, "") + location.search + location.hash;
    s.prop54 = s.prop54.replace(/^\//, "");


    // Directory#1 - #7
    // prop 55 - prop62

    s.arrPath = s.strPathNew.split("/");
    for (s.intIdx = 55; s.intIdx < 62; s.intIdx++) {
        if (s.arrPath[s.intIdx - 55]) {
            eval("s.prop" + (s.intIdx) + " = '" + s.arrPath[s.intIdx - 55] + "';");
        }
    }
    ...
    s.prop62 = location.search + location.hash;
    s.prop62 = s.prop62.replace(/^\?/, "");
    ...
    s.prop4 = s.channelExtract("/", 5, s.pageName, 1);
    s.prop5 = s.channelExtract("/", 6, s.pageName, 1);
    s.prop6 = s.channelExtract("/", 7, s.pageName, 1);
    ...
    // File Download
    // prop35, prop64, prop67, eVar3 ,event9 ,event10
    s.dLinkEXT = s.linkDownloadFileTypes.replace(/,/g,"|");
    s.objLh = s.linkHandler(s.dLinkEXT,'d');
    if (s.objLh) {
        s.linkTrackVars = "events,prop46,prop36,prop37,prop38,prop39,prop40,prop41,prop42,prop47,prop48,prop49,prop50,prop51,prop35,prop54,prop67";
		s.prop54 = s.objLh;
        s.prop35 = s.objLh.substring(s.objLh.lastIndexOf("/") + 1);

        s.prop67 = "D=pageName";
		
        
        // Plugin Example: getPercentPageView v1.71 multiple values 

        s.prop46 = s.getPreviousValue(s.pageName,'s_ppn'); // Previous Page 
    
        var ppv = s.getPercentPageViewed(s.pageName); // Get array of data on prev page % viewed

        s.prop36 = s.channelExtract(":", 4, s.prop46, 1);  // Previous Directory #2
        s.prop37 = s.channelExtract(":", 5, s.prop46, 1);  // Previous Directory #3
        s.prop38 = s.channelExtract(":", 6, s.prop46, 1);  // Previous Directory #4
        s.prop39 = s.channelExtract(":", 7, s.prop46, 1);  // Previous Directory #5
        s.prop40 = s.channelExtract(":", 8, s.prop46, 1);  // Previous Directory #6
    
        s.prop41 = ppv[3]; // Total Number of Vertical Pixels
        //s.prop42 = ppv[9];  device orientation: initial and final (final only present if rotation occurred)       
        s.prop47 = ppv[1]; // Total % of Page Viewed
        s.prop49 = ppv[2]; // Percent viewed on initial load

        s.prop50 = s.prop47 - s.prop49; // Scrolling %
        s.prop51 = s.prop49 + "%|" + s.prop50 + "%|";   // Initial loadind + Scrollig   
        s.prop48 = s.prop51 + s.prop41 + "pixels|" + s.prop46;   // initial loadind + Scrollig + Length + PageName
    ...
    // Partner Link Click
    // eVar17 , event14
    s.extLh = s.exitLinkHandler();
    if (s.extLh) {
        
        s.linkTrackVars = "events,eVar16,prop46,prop36,prop37,prop38,prop39,prop40,prop41,prop42,prop47,prop48,prop49,prop50,prop51,prop54";
        s.prop54 = s.extLh;
        ...
        s.eVar16 = s.extLh.match(/https?:\/\/([^\/]+)/)[1];
    ...
    if (location.pathname.match(/\/lib\/(.*?)\/ebook\//)) {
    ...
    s.prop70 = s.getDaysSinceLastVisit('s_lv');
 ...
 */
s.channelExtract = new Function("d", "p", "u", "pv", "" + "var s=this,v='';u=u?u:(s.pageURL?s.pageURL:s.wd.location);if(u=='f'" + ")u=s.gtfs().location;u=u+'';li=u.lastIndexOf(d);if(li>0){u=u.substr" + "ing(0,li);var i,n,a=s.split(u,d),al=a.length;if(al<p){if(pv==1) p=a" + "l;else return '';}for(i=0;i<p;i++){n=a[i];v=v+n+d;}return v}return " + "'';");
/*
...
s.visitorNamespace = "renesas"
s.dc = "122"
s.trackingServer="renesas.122.2o7.net"
